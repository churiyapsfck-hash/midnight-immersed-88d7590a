...
export const getPassPdf = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Header
    page.drawText("ILLUMINATI 3.0", {
      x: 30, y: 520, size: 22, font, color: rgb(1, 1, 1),
    });
    page.drawText("— BY INVITATION", {
      x: 30, y: 500, size: 8, font: mono, color: rgb(0.75, 0.15, 0.15),
    });
    ...
    const qrSize = 220;
    page.drawRectangle({
      x: (420 - qrSize) / 2 - 10, y: 240,
      width: qrSize + 20, height: qrSize + 20,
      color: rgb(1, 1, 1),
    });
    page.drawImage(qrImg, {
      x: (420 - qrSize) / 2, y: 250, width: qrSize, height: qrSize,
    });
    ...
    drawLine("USER ID", prof?.user_code ?? "—", 170);
    drawLine("PASS", `${b.pass_type.toUpperCase()} · ${b.category.toUpperCase()}`, 130);

    // Footer
    page.drawText("AUG 3 · MARQUEE CLUB", {
      x: 30, y: 60, size: 9, font: mono, color: rgb(0.7, 0.7, 0.75),
    });
    page.drawText("Present this pass at the gate. One-time entry.", {
      x: 30, y: 45, size: 9, font: serif, color: rgb(0.6, 0.6, 0.65),
    });

    const bytes = await pdf.save();
    // Return as base64 so it can traverse the RPC serialization boundary safely.
    let bin = "";
    for (let i = 0; i < bytes.length; i++) bin += String.fromCharCode(bytes[i]);
    const base64 = btoa(bin);
    return { base64, filename: `illuminati-pass-${prof?.user_code ?? b.id.slice(0, 8)}.pdf` };
  });